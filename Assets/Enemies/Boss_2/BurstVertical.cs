...
public class BurstVertical : State
{
    [SerializeField] private ParticleSystem verticalParticle; // Assign in Inspector
    ...
    [SerializeField] private Vector3 movementAxis = Vector3.left; // Axis for movement (e.g., left/right = Vector3.right)
    ...
    {
        Debug.Log("Entering Burst Vertical State");

        isMoving = true;
        timer = Random.Range(minDuration, maxDuration);
        if (verticalParticle != null)
        {
            verticalParticle.gameObject.SetActive(true);
            verticalParticle.Play(); // Start the particle effect
    ...
    public override void Exit()
    {
        ...
        if (verticalParticle != null)
        {
            verticalParticle.Stop(); // Stop the particle effect
        }
