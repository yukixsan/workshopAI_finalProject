...
public class VerticalState : State
{
    [SerializeField] private ParticleSystem verticalParticles; // Assign in Inspector
    ...
    {
        Debug.Log("Entering Vertical State");

        isMoving = true;
        timer = Random.Range(minDuration, maxDuration);
        if (verticalParticles != null)
        {
            verticalParticles.gameObject.SetActive(true);
            verticalParticles.Play(); // Start the particle effect
        }
    }

    public override void Exit()
    {
        Debug.Log("Exiting Vertical State");

        isMoving = false;

        if (verticalParticles != null)
        {
            verticalParticles.Stop(); // Stop the particle effect
        }
    }
    ...
    public override void Update()
    {
        ...
        if (timer <= 0f)
        {
            ...
            stateMachine.ChangeState<HorizontalState>();
        }
        if (isMoving)
        {
            Move();
